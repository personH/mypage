...
/*
 * Modified mPicid original in AOSP.
 */
public class CropImageActivity extends MonitoredActivity implements OnDataRetrieveListener {
    ...
    private String mPicPath;//原始图片的路径
    private String mPicid;//从哪个activity来
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case CHECK_KEY_SUC:
                    ...
                    upload(userid, mPicPath, keyfield);
                    break;
                case CHECK_KEY_FAIL:
    ...
    private void setupFromIntent() {
        ...
        if (sourceUri != null) {
            ...
            try {
                ...
                LogUtils.putLog(mContext, "Error reading image: " + e.getMessage());
                setResultException(e);
            } catch (OutOfMemoryError e) {
                LogUtils.putLog(mContext, "OOM reading image: " + e.getMessage());
                setResultException(e);
            } finally {
    ...
    private Bitmap decodeRegionCrop(Bitmap croppedImage, Rect rect) {
        ...
        try {
            ...
            LogUtils.putLog(mContext, "Error cropping image: " + e.getMessage());
            finish();
        } catch (OutOfMemoryError e) {
            LogUtils.putLog(mContext, "OOM cropping image: " + e.getMessage());
            setResultException(e);
        } finally {
            CropUtil.closeSilently(is);
        }
    ...
    private void saveOutput(Bitmap croppedImage) {
        if (saveUri != null) {
            ...
            try {
                ...
                setResultException(e);
                LogUtils.putLog(mContext, "Cannot open file: " + saveUri);
            } finally {
     ...
     * 保存背景图片
     */
    private void saveBackground(Bitmap bmpBj) {
        String filePath = TxNetwork.APP_DOWNLOADFILE_PATH + "cropped.jpg";//裁剪完了的图片路径
        if (bmpBj == null) {//图片不存在
            Toast.makeText(mContext, R.string.pic_cut_error, Toast.LENGTH_SHORT).show();
        } else {
            SharedUtils.saveIsBackGroundOut(mContext, true);
            SharedUtils.saveBackGroundOut(mContext, filePath);
            if (null != mPicid && !"".equals(mPicid)) {//from preview
                SkinUtils.addNewDownloadedTheme(mContext, filePath, getResources().getString(R.string.diy), 0, mPicid);//添加到已下载
                finish();
                //设置当前皮肤为自定义
            ...
            } else {//from first select pic
                Intent data = new Intent();
                data.putExtra("picpath", mPicPath);
                this.setResult(MainActivity.ADD_LOCAL_THEME, data);
                this.finish();
                //先添加到本地自定义表,
                SkinUtils.insertDiyTheme(mContext, mPicPath);//
                //上传选择的原图,上传成功会自动设为当前皮肤
                String userid = TxNetworkUtil.getUserId(mContext);//获取用户id;
                if (StringUtil.isNotNull(userid)) {
                    String aesKey = TxNetworkUtil.getUserKey(mContext);
                    if (aesKey != null) {
                        upload(userid, mPicPath, aesKey);
                    } else {
                        reqAppkey();
                    }
                } else {
     ...
     * @param aesKey
     */
    private void upload(String userid, String picPath, String aesKey) {

        String server_url = ConstantPool.SERVER_URL + ConstantPool.UPLOAD_PIC;

        String deviceid = TxNetworkUtil.getIMEIId(mContext);
        String sign = MD5Util.md5Hex(deviceid + userid + aesKey, "UTF-8").toUpperCase();
        server_url = server_url + "?deviceid=" + deviceid + "&sign=" + sign;
        UploadUtil uploadUtil = UploadUtil.getInstance();
        uploadUtil.setOnUploadProcessListener(onUploadProcessListener);  //设置监听器监听上传状态
        uploadUtil.uploadFile(mContext, picPath, server_url);
    }
    ...
    UploadUtil.OnUploadProcessListener onUploadProcessListener = new UploadUtil.OnUploadProcessListener() {
        ...
        public void onUploadDone(int responseCode, String message) {
            switch (responseCode) {
                case UploadUtil.UPLOAD_SUCCESS_CODE:
                    Toast.makeText(mContext, R.string.upload_pic_suc, Toast.LENGTH_SHORT).show();
                    break;
        ...
        public void onUploadProcess(int uploadSize) {
            //Toast.makeText(mContext, R.string.upload_pic_ing, Toast.LENGTH_SHORT).show();
        }
