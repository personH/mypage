...
public class DragSortController extends SimpleFloatViewManager implements View.OnTouchListener, GestureDetector.OnGestureListener {
    ...
    public DragSortController(DragSortListView dslv, int dragHandleId, int dragInitMode,
        ...
        mVibrator = (Vibrator) dslv.getContext().getSystemService(Context.VIBRATOR_SERVICE);
    ...
    public boolean startDrag(int position, int deltaX, int deltaY) {
        ...
        if (mSortEnabled && !mIsRemoving) {
            dragFlags |= DragSortListView.DRAG_POS_Y | DragSortListView.DRAG_NEG_Y;//按位或运算
        }
        if (mRemoveEnabled && mIsRemoving) {
            dragFlags |= DragSortListView.DRAG_POS_X;//dragFlags =dragFlags|DragSortListView.DRAG_POS_X
    ...
    public boolean onDown(MotionEvent ev) {
        ...
        mHitPos = startDragPosition(ev);//当前按下的那一项的位置
    ...
    public void onLongPress(MotionEvent e) {
        ...
        if (mHitPos != MISS && mDragInitMode == ON_LONG_PRESS) {
            mDslv.performHapticFeedback(HapticFeedbackConstants.LONG_PRESS);//反馈效果
            this.mDragInitMode = ON_DOWN;//修改拖动模式为按下拖动

            mDslv.setEditing(true);

            mVibrator.vibrate(50);
            LinearLayout linearLayout = (LinearLayout) mDslv.getParent();
            linearLayout.findViewById(R.id.add_type).setVisibility(View.VISIBLE);
            linearLayout.invalidate();

            for (int i = 0; i < mDslv.getChildCount(); i++) {
                View childView = mDslv.getChildAt(i);
...
//                dragGridAdapter.deleteItem(dragGridAdapter.getCount() - 1);

                childView.setAlpha((float) 0.6);//SDK版本问题
                ...
                TextView typeNmae = (TextView) childView.findViewById(R.id.typeName);
                //EditText editText=new EditText();
            }
            //onEditListener.getEditState(true);
    ...
    public interface OnEditListener {
        ...
        public void getEditState(boolean flag);
    }
