...
public class DragSortListView extends ListView {
     ...
     *
     * @attr name com.jit.demo.dslv:drag_enabled
     */
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (isEditing) {
            for (int i = 0; i < getChildCount(); i++) {
                ...
                childView.setAlpha((float) 0.8);
                childView.findViewById(R.id.expand).setVisibility(View.INVISIBLE);
                childView.findViewById(R.id.delete).setVisibility(View.VISIBLE);

                DragGridView dragGridView = (DragGridView) childView.findViewById(R.id.dragGridView);

                EditText typeEdit = (EditText) childView.findViewById(R.id.typeName);
                typeEdit.setEnabled(true);
                //typeEdit.setSelection(typeEdit.getText().length());//  光标移动最后
                typeEdit.setBackgroundResource(R.drawable.bg_edittext);
                //dragGridView.setDragResponseMS(10);
//                int count = dragGridView.getAdapter().getCount();
//                DragAdapter dragAdapter = (DragAdapter) dragGridView.getAdapter();
//                //dragAdapter.removeItem(count - 1);
//                dragGridView.setAdapter(dragAdapter);
                dragGridView.setEditing(true);
    ...
    private void adjustItem(int position, View v, boolean invalidChildHeight) {
        ...
        if (isEditing()) {//在draggridview收起的时候,进行拖动会出现数据交错混乱的问题,当处于编辑状态的时候,默认设置成显示状态
            DragGridView dragGridView = (DragGridView) v.findViewById(R.id.dragGridView);
            dragGridView.setVisibility(vis);
        }
     ...
     * your own is not passed, the default {@link com.jit.demo.dslv.SimpleFloatViewManager}
    ...
    public interface FloatViewManager {
         ...
         *                      and the offset provided in {@link com.jit.demo.dslv.DragSortListView#startDrag}.
     ...
     * {@link com.jit.demo.dslv.DragSortListView.DragScrollProfile}.
     *
     * @param ssp
     */
     ...
     * {@link com.jit.demo.dslv.DragSortListView#setDragScrollProfile(DragScrollProfile)} to
     * set custom profile.
     *
     * @author heycosmo
     */
    ...
    private class DragScroller implements Runnable {
        ...
        public void startScrolling(int dir) {
            if (!mScrolling) {
                // Debug.startMethodTracing("com.jit.demo.dslv-scroll");
                mAbort = false;
                mScrolling = true;
    ...
    public void setmAdapterWrapper(AdapterWrapper mAdapterWrapper) {
