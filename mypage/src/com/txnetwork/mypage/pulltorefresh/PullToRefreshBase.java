...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		// Styleables from XML
		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.PullToRefreshGridView);

		if (a.hasValue(R.styleable.PullToRefreshGridView_ptrMode)) {
			mMode = Mode.mapIntToValue(a.getInteger(R.styleable.PullToRefreshGridView_ptrMode, 0));
		}

		if (a.hasValue(R.styleable.PullToRefreshGridView_ptrAnimationStyle)) {
			mLoadingAnimationStyle = AnimationStyle.mapIntToValue(a.getInteger(
					R.styleable.PullToRefreshGridView_ptrAnimationStyle, 0));
		}
		 ...
		 * Styleables from XML
		 */
		if (a.hasValue(R.styleable.PullToRefreshGridView_ptrRefreshableViewBackground)) {
			Drawable background = a.getDrawable(R.styleable.PullToRefreshGridView_ptrRefreshableViewBackground);
			if (null != background) {
				mRefreshableView.setBackgroundDrawable(background);
			}
		} else if (a.hasValue(R.styleable.PullToRefreshGridView_ptrAdapterViewBackground)) {
			Utils.warnDeprecation("ptrAdapterViewBackground", "ptrRefreshableViewBackground");
			Drawable background = a.getDrawable(R.styleable.PullToRefreshGridView_ptrAdapterViewBackground);
			if (null != background) {
				mRefreshableView.setBackgroundDrawable(background);
			}
		}

		if (a.hasValue(R.styleable.PullToRefreshGridView_ptrOverScroll)) {
			mOverScrollEnabled = a.getBoolean(R.styleable.PullToRefreshGridView_ptrOverScroll, true);
		}

		if (a.hasValue(R.styleable.PullToRefreshGridView_ptrScrollingWhileRefreshingEnabled)) {
			mScrollingWhileRefreshingEnabled = a.getBoolean(
					R.styleable.PullToRefreshGridView_ptrScrollingWhileRefreshingEnabled, false);
		}
	...
	public static enum Mode {
		 ...
		 * {@link com.example.testpulltorefresh.pulltorefresh.PullToRefreshBase#setRefreshing() setRefreshing()}.
		 */
	 ...
	 * {@link com.example.testpulltorefresh.pulltorefresh.PullToRefreshAdapterViewBase#setOnLastItemVisibleListener}.
	 *
	 * @author Chris Banes
	 */
	 ...
	 * (such as sounds). See (
	 * {@link com.example.testpulltorefresh.pulltorefresh.PullToRefreshAdapterViewBase#setOnPullEventListener}.
	 *
	 * @author Chris Banes
	 */
